...
export default apiPageHandler(
    ...
    },
    {
        POST: async (request) => {
            ...
            const data = cleanString(request.getData()?.user_summary ?? "");

            const messages: ChatCompletionMessage[] = [
                {
                    role: "system",
                    content: [
                        `You will be provided with content from a user. You will need to create a list of skills the user might have. The skills will be provided in the following format:`,
